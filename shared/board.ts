...
// 실루엣 종류(pikachu/charizard/unown/strawhat/whitebeard/laboon/zeus/sudowoodo/skull/apple)는 다중 해상도 비트맵을
...
export const MASK_CANVAS: Partial<Record<MaskKind, Record<DifficultyKey, { rows: number; cols: number }>>> = {
  pikachu: {
    easy: { rows: 10, cols: 13 },
    normal: { rows: 12, cols: 16 },
    hard: { rows: 14, cols: 18 },
  },
  unown: {
    easy: { rows: 10, cols: 13 },
    normal: { rows: 12, cols: 16 },
  ...
  // 잉어킹은 프리셋이 초급 고정이라 10×13 만 exact — normal/hard 는 최근접 확대 폴백(프리셋 미사용)
  magikarp: {
    easy: { rows: 10, cols: 13 },
    normal: { rows: 12, cols: 16 },
    hard: { rows: 14, cols: 18 },
...
// 손으로 그린 실루엣 비트맵('#'=타일, 그 외=빈칸). 정확한 캔버스 치수의 변형을 우선하고,
// 일치하는 변형이 없으면 sampleSilhouette 가 최근접(nearest-neighbor)으로 정합한다.
// 모든 실루엣은 유효 칸 수를 짝수로 그렸고(makeMask 짝수 보정으로 한 번 더 방어), 흩어진 칸도 빈 칸을
// 통한 경로 연결이 가능하도록 충분히 덩어리진 형태로 유지한다.
// 피카츄 머리의 양 귀와 눈 구멍을 난이도별 캔버스에 맞춰 표현한다.
// 안농 물음표 폼은 갈고리와 점의 2개 섬이며, 두 섬은 빈칸 경로로 연결된다.
const SILHOUETTE_VARIANTS: Partial<Record<MaskKind, string[][]>> = {
  ...
  // 안농(레퍼런스 재적용 2026-07-11): 링(내부 구멍) + 대각 팔 4개 + 중앙 눈(의도된 분리 섬).
  // 프리셋이 일반 고급 고정이라 14×18 만 exact — easy/normal 은 최근접 폴백(프리셋 미사용).
  unown: [
    [
      "##..............##",
      ".##............##.",
  ...
  charizard: [
    ...
    // exact 12×16/10×13 — 최근접 다운샘플이 뿔·눈을 깨뜨려 직접 도안 (형상 감사 2026-07-11)
    [
      ".....##..##.....",
      ".#...######...#.",
      ".##..#.##.#..##.",
  ...
  laboon: [
    // 상하 반전 도안 — 꼬리 V 가 위를 향하게 (플레이 피드백 2026-07-11). 채움 수 불변.
    // easy 10×13 (58칸, 꼬리 위 고래)
    [
      ".............",
      "......##..##.",
      ".......####..",
  ...
  sudowoodo: [
    ...
    // exact 12×16/10×13 — 다운샘플이 팔 브리지를 찢던 문제 교정 (형상 감사 2026-07-11)
    [
      ".......##.......",
      "......####......",
      ".##...####...##.",
  ...
  skull: [
    ...
    // exact 12×16(손질)/10×13 — 뼈 끝 고립 조각·하단 노이즈 교정 (형상 감사 2026-07-11)
    [
      ".##..........##.",
      "..##..#####..##.",
      "..############..",
